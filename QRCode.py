...
#     qr_label.place(relx=0.5, rely=0.6, anchor=tk.CENTER)  # Adjusted relx to 0.6 for lower placement
...
# tk.Label(root, text="Enter Text or URL:", fg="black", bg="#ffdc5b", font=("Arial", 18)).pack(pady=6)

# # Entry with drag-and-drop
...
# theme_var = tk.StringVar(value="Minimal White")
...
# qr_label.pack(pady=15)  # Added padding to move it downward
...
#         qr_label.config(image=img)
#         qr_label.image = img
#         qr_label.place(relx=0.5, rely=0.6, anchor=tk.CENTER)  # Adjusted placement
#     except Exception as e:
#         messagebox.showerror("Error", f"Failed to load QR image: {e}")
...
# def on_drop(event):
#     dropped_text = event.data.strip("{}")  # Remove curly brackets around file paths (Windows issue)
#     entry.delete(0, tk.END)
...
# root.title("QR Code Generator")
# root.geometry("400x500")
# root.configure(bg="#ffdc5b")  # Background color
...
# theme_var = tk.StringVar(value="Cyber Theme")  # Only set once
...
# # Function to display QR with animation
# def show_qr_animation(img_path):
#     img = Image.open(img_path)
#     img = ImageTk.PhotoImage(img)
#     qr_label.config(image=img)
